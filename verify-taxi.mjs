...
const places=[
 ...
 {name:'Portello',tag:'Università',x:50,z:60},
 {name:'Arcella',tag:'Quartiere',x:70,z:80},
 {name:'Stadio Euganeo',tag:'Stadio',x:90,z:100}
];
const destinations=taxiDestinations(places,{x:-100,z:-100},{x:100,z:100});
for(const name of ['Prato della Valle','Piazza dei Signori','Portello','Aeroporto','Arcella','Capolinea tram sud (Albignasego)','Stadio','Ponte San Nicolò','Vigonza','Zona Industriale'])assert(destinations.some(p=>p.name===name),name+' destination');
assert.equal(destinations.length,10,'destination menu contains the current ten destinations');
...
// Test the current modular dispatcher, rather than removed legacy globals.
const stubPathfinder={nearestRoad:pos=>({x:pos.x,z:pos.z,y:0,yaw:0}),route:(from,to)=>[{x:from.x,z:from.z},{x:to.x,z:to.z}]};
...
assert(plan&&plan.path.length>=2,'dispatcher plans a physical pickup');
assert(Math.hypot(plan.spawn.x,plan.spawn.z)>=28,'taxi does not spawn on player');
...
assert(pickup,'current dispatcher completes its pickup');
assert.equal(new TaxiPathfinder({graph:{},terrain,bounds:{x:-10,z:-10,w:20,h:20}}).valid({x:50,z:0}),false,'out-of-bounds destination rejected');
...
for(const [label,expected] of [
 ['physical dispatch','function dispatchPhysicalTaxi('],
 ['dispatch planning','taxiDispatcher.planDispatch(state)'],
 ['taxi update','function updateTaxi(dt)'],
 ['talk range','taxiDriverNPC?.canInteract(state,3.3)'],
 ['destination selection','taxiMenuController.startTaxiTransition('],
 ['transition','await taxiSystem.travel('],
...
])assert(game.includes(expected),label+' is wired into runtime');
