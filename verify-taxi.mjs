...
const report={fare:{minimum:taxiFare({x:0,z:0},{x:20000,z:20000}),maximum:taxiFare({x:0,z:0},{x:20000,z:20000})},destinations:destinations.map(p=>p.name),route:{arrived,blocked,finalZ:car.z,maxStep},live:{arrived:liveTaxi.phase==='at-destination',driverSunglasses:true,talkRange:true},loading:{pixelFrames:8,factsClickable:true,coreReadyGate:true,coreRadius:72,prefetchRadius:260,streamFocusRadius:360},map:{zoomPanCoordinateFix:true,spaceConfirmsTrip:true,singleChargeGuard:true},missionTaxi:{singleActiveTransaction:true},testWallet:50000};
fs.writeFileSync('docs/taxi-results.json',JSON.stringify(report,null,2)+'\n');
console.log('PASS physical taxi, single-charge confirmation, mission taxi transaction guard and core-ready gate',report);
