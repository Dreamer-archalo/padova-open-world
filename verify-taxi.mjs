...
assert(game.includes("CHIAMA TAXI ABUSIVO"));assert(game.includes("E · PARLA"));assert(game.includes("SCEGLI TU"));assert(game.includes('world.coreReady(')&&game.includes('world.prefetch('),'loading gates transfer on core streaming');assert(!game.includes('travel(PLACES['),'map no longer teleports directly');
...
const report={fare:{minimum:taxiFare({x:0,z:0},{x:1,z:1}),maximum:taxiFare({x:0,z:0},{x:20000,z:20000})},destinations:destinations.map(p=>p.name),route:{arrived,blocked,finalZ:car.z,maxStep},live:{arrived:liveTaxi.phase==='at-destination',driverSunglasses:true,talkRange:true},loading:{pixelFrames:8,factsClickable:true,coreReadyGate:true}};
