...
const document={getElementById(id){assert(els.has(id),'missing DOM id '+id);return els.get(id);},querySelectorAll:()=>[],addEventListener(){},createElement:element};
globalThis.document=document;
const ctx=vm.createContext({THREE,...core,...worldModule,...movement,BuildingModels,document,window:{addEventListener(){}},localStorage:{getItem:()=>null,setItem(){}},console,Math,JSON,Set,Map,Number,Array,Float32Array,Uint8Array,devicePixelRatio:1,innerWidth:1440,innerHeight:900,requestAnimationFrame(){},location:{reload(){}}});
...
vm.runInContext(`data=testData;scene=new THREE.Scene();world=new CityWorld(scene,data);graph=makeRoadGraph(data.roads);player=createPerson();camera=new THREE.PerspectiveCamera();sun=new THREE.DirectionalLight();marker=new THREE.Group();state.ready=true;state.started=true;createPopulation();followYaw=state.yaw;`,ctx);
const t=vm.runInContext('({state,keys,cars,people,world,scene,player,camera,clock,movePlayer,updateCamera,updateUI,toggleVehicle,beginMission,cancelMission,updateMission,clearPolice,simulate})',ctx);
...
t.toggleVehicle();assert.equal(t.state.mode,'car');assert.equal(t.state.y,0);
...
for(const hz of [30,60,144]){
  Object.assign(t.state,start,{speed:0,y:0,vy:0,elapsed:0});t.clock.reset();t.keys.clear();t.keys.add('KeyW');t.keys.add('KeyA');
...
t.keys.clear();t.keys.add('Space');t.movePlayer(1/60);assert(t.state.y>0);t.keys.clear();for(let i=0;i<120;i++)t.movePlayer(1/60);assert.equal(t.state.y,0);
...
console.log('PASS: swept movement, camera obstruction, car extent, 30/60/120/144 Hz clock, actual controller across frame rates, jump, delivery, missions, strict HUD IDs, GLB parsing/replacement/unload/failure fallback.');
