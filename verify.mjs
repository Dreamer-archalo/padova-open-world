...
const targets=PLACES.map(p=>{const t=safeDryRoad(p,graph,world.collision,terrain,VEHICLES.mito);assert(t,'no spawn '+p.name);const y=terrain.height(t.x,t.z);assert(!collides(t.x,t.z,VEHICLES.mito.width/2,world.collision,y),'blocked '+p.name);return {...t,y,name:p.name};});
...
const spawn=targets[0];assert(!collides(spawn.x,spawn.z,VEHICLES.mito.width/2,world.collision,spawn.y));
