...
const scene=new THREE.Scene();const world=new CityWorld(scene,data),graph=makeRoadGraph(data.roads);
const targets=PLACES.map(p=>{const t=safeRoadPoint(p,graph,world.collision,1.3);assert(t,'no spawn '+p.name);assert(!collides(t.x,t.z,1.3,world.collision),'blocked '+p.name);return {...t,name:p.name};});
