...
for(const ramp of ramps){
 ...
 assert(launched,'ramp launch '+JSON.stringify({name:ramp.name,x:ramp.x,z:ramp.z}));assert(landed,'ramp landing '+JSON.stringify({name:ramp.name,x:ramp.x,z:ramp.z}));assert(flightFrames>15,'visible airborne interval');assert(maxY>Math.max(...ramp.topY)+.35,'jump clears ramp lip');
 assert(Math.abs(actor.y-groundContact(t.terrain,actor.x,actor.z,actor.y).y)<.05,'lands on mapped ground');
 results.push({name:ramp.name,x:ramp.x,z:ramp.z,flightFrames,maxY,landingX:actor.x,landingZ:actor.z});
}
