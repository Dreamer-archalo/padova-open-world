...
export class CityWorld{
 ...
 build(key){const ch=this.chunks.get(key);if(!ch)return;const g=new THREE.Group(),wallBatches={historic:new GeometryBatch(),modern:new GeometryBatch(),industrial:new GeometryBatch()},roofs=new GeometryBatch(),surfaces=new GeometryBatch();
  for(const b of ch.buildings){if(b.modelActive||b.authoredLandmark)continue;const p=b.p,h=b.h,central=Math.hypot(b.cx*.82,b.cz)<1550,historic=central||['church','chapel','basilica','historic','civic','museum','theatre'].includes(b.t),industrial=['industrial','warehouse','hangar','commercial','retail','supermarket','mall'].includes(b.t),walls=industrial?wallBatches.industrial:historic?wallBatches.historic:wallBatches.modern;walls.baseY=roofs.baseY=b.minY;for(let i=0;i<p.length;i++){const a=p[i],q=p[(i+1)%p.length],len=Math.hypot(q[0]-a[0],q[1]-a[1]);walls.quad([a[0],this.terrain?Math.min(.15,this.terrain.groundHeight(a[0],a[1])-b.minY-.15):.15,a[1]],[q[0],this.terrain?Math.min(.15,this.terrain.groundHeight(q[0],q[1])-b.minY-.15):.15,q[1]],[q[0],h,q[1]],[a[0],h,a[1]],wallColors[b.c],Math.max(1,Math.round(len/(industrial?7:historic?6.6:13.8))),Math.max(1,Math.round(h/(industrial?5:historic?6.2:9.3))));}surface(roofs,p,h+.08,industrial?col('#8b8c86'):roofColors[b.c]);if(central&&b.c===0&&h>8)portico(roofs,p);
  ...
  for(const a of ch.areas)surface(surfaces,a.p,a.k==='water'?water:(x,z)=>height(x,z)-.02,col(a.k==='water'?'#639b9c':a.k==='pitch'?'#7d9e72':'#789961'));
  for(const r of ch.water)strip(surfaces,r.a,r.b,r.w,water,col('#659b9e'));
  for(const s of ch.roads){const k=s.road.k,ped=['pedestrian','footway','path','cycleway','steps'].includes(k),mx=(s.a[0]+s.b[0])/2,mz=(s.a[1]+s.b[1])/2,central=Math.hypot(mx*.82,mz)<1550,bridge=s.road.b,edge=bridge?'#c2b49a':ped&&central?'#d0c2a4':'#c3bca8',road=ped?(central?'#bdae91':'#b7b09a'):(central?'#696d69':'#6c7472');
   const roadY=(x,z)=>{if(bridge&&terrain){const q=nearestOnSegment(x,z,s.a,s.b);return height(q.x,q.z)+.025;}return height(x,z)+.025;};
   strip(surfaces,s.a,s.b,s.w+(bridge?3:1.5),(x,z)=>roadY(x,z)-.02,col(edge));strip(surfaces,s.a,s.b,s.w,roadY,col(road));
   if(bridge){const dx=s.b[0]-s.a[0],dz=s.b[1]-s.a[1],length=Math.hypot(dx,dz),count=Math.max(1,Math.ceil(length/8));if(length>.01)for(const side of [-1,1])for(let i=0;i<count;i++){const nx=-dz/length*(s.w/2+1)*side,nz=dx/length*(s.w/2+1)*side,ax=s.a[0]+dx*i/count+nx,az=s.a[1]+dz*i/count+nz,bx=s.a[0]+dx*(i+1)/count+nx,bz=s.a[1]+dz*(i+1)/count+nz,ay=roadY(ax,az),by=roadY(bx,bz);surfaces.quad([ax,ay,az],[bx,by,bz],[bx,by+1.1,bz],[ax,ay+1.1,az],col(central?'#bbab91':'#a7aaa6'));}}
...
export function createPerson(color='#c6ad82',variant=0){const g=new THREE.Group(),skins=['#ceaa87','#a8785e','#e0b99a','#805944'],hair=['#403d31','#6b4c32','#242a2b','#9b8059'],pants=['#394749','#3d4965','#564a43','#26383c'],skin=skins[variant%4];g.scale.setScalar(.92+(variant%5)*.035);const hips=new THREE.Group();for(const side of [-1,1]){const leg=new THREE.Group();leg.position.set(side*.16,.82,0);box(leg,pants[variant%4],0,-.34,0,.22,.68,.25);box(leg,'#273033',0,-.72,.065,.24,.12,.4);hips.add(leg);}g.add(hips);box(g,color,0,1.12,0,.55,.62,.34);primitive(g,sphereGeo,skin,0,1.64,0,.19,.23,.2);primitive(g,sphereGeo,hair[variant%4],0,1.8,-.015,.2,.10,.2);const arms=new THREE.Group();for(const side of [-1,1]){const arm=new THREE.Group();arm.position.set(side*.36,1.36,0);box(arm,color,0,-.25,0,.17,.49,.2);primitive(arm,sphereGeo,skin,0,-.56,0,.09,.12,.1);arms.add(arm);}g.add(arms);if(variant%4===0)box(g,'#5b4035',0,1.12,-.24,.36,.43,.16);if(variant%7===0)primitive(g,cylinderGeo,'#38464c',0,1.91,0,.24,.08,.24);g.userData.hips=hips;g.userData.arms=arms;return g;}
