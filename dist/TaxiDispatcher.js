...
export class TaxiDispatcher {
  ...
  roadNear(pos, radius = 180) {
    return this.pathfinder.nearestRoad(pos, {maxRadius: radius, maxCandidates: 1400, maxMs: 12});
  }
  ...
  pickup(player) {
    for (const metres of [10, 7, 4, 0]) {
      const p = {
        x: player.x + Math.sin(player.yaw) * metres,
        z: player.z + Math.cos(player.yaw) * metres,
      };
  ...
  route(from, to) {
    return this.pathfinder.route(from, to, {maxSteps: 12000, maxMs: 20});
  }
  ...
  planDispatch(player) {
    ...
    for (const radius of radii) {
      for (const offset of rearAngles) {
        ...
        const probe = {
          x: player.x + Math.sin(yaw) * radius,
          z: player.z + Math.cos(yaw) * radius,
        };
