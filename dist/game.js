...
async function startUnifiedRegion(){
 ...
 try{
  ...
  unifiedMap=new UnifiedMap($('fullmap'),mapBase,minBounds,map);
  ...
  control('unifiedMapHere',()=>unifiedMap.centerOn(state.x,state.z,Math.max(4,unifiedMap.zoomLevel)));
...
function regionalFastTravel(p){
 ...
 regionalWorld.update(state,0);closeDialogs();toast('Spostamento rapido · '+p.name,4);
}
