...
const state={ready:false,started:false,paused:false,mode:'foot',x:-150,z:-49,y:0,yaw:-.5,speed:0,vy:0,health:100,money:0,wanted:0,escape:0,busted:0,car:null,mission:null,waypoint:null,route:[],camera:0,quality:'low',sound:false,elapsed:0,distance:0,jobs:0,character:'fede',parachuting:false,respawnHome:false};
let characterPicker;const cameraRig=new CameraRig();let signals,trams,incidents,gameplay,parachute,speedCameras,taxi=null,taxiMapPick=false;const waterRecovery=new WaterRecovery();let terrain,districts;
...
function selectCharacter(id){state.character=normalizeCharacter(id);const old=player;player=createCharacter(state.character);player.position.set(state.x,state.y,state.z);player.rotation.y=state.yaw;scene.add(player);if(old)scene.remove(old);}
...
function respawnAtVilla(){
 const wreck=state.car;if(wreck){resetGroundMotion(wreck);wreck.health=0;wreck.speed=0;wreck.mesh.visible=false;wreck.destroyedUntil=state.elapsed+15;}
...
function restyleTraffic(c,zone){if(c.police||c.parked)return;const legacy=DISTRICTS[zone].vehicles,style=Math.random()<.2?legacy[Math.floor(Math.random()*legacy.length)]:chooseTrafficStyle(zone,Math.random,c.road);if(style===c.style)return;
 if(!trafficModels.has(style)){const color=['#a92731','#667d7d','#c9c1ac','#687688'][Math.max(0,vehicleStyles.indexOf(style))%4],m=SPECIAL_VEHICLES[style]?createSpecialVehicle(style):compactCar(NPC_VEHICLES[style]?createNPCCar(style,color):['mito','cinquecento','scooter','motorcycle','truck'].includes(style)?createVehicle(style,color):createCar(color,false,style));trafficModels.set(style,m);}
 const old=c.mesh;scene.remove(old);previousActors.delete(old);c.mesh=trafficModels.get(style).clone(true);scene.add(c.mesh);c.style=style;c.spec=VEHICLES[style];c.name=c.spec.name;c.rider=null;c.damageVisual=null;installVehicleDamage(c);if(isBike(style)||VEHICLES[style]?.bike){c.rider=createRider();c.mesh.add(c.rider);}
}
function placeTraffic(c,minD=100,maxD=550){const id=goodNearbyNode(state,minD,maxD,c.spec,c.style);if(id===null){c.mesh.visible=false;return;}const n=graph.nodes[id];c.road=n.edges[0]?.road;restyleTraffic(c,districts?.at(n.x,n.z,n.edges[0]?.road)||'urban');c.x=n.x;c.z=n.z;c.prev=id;c.target=n.edges[Math.floor(Math.random()*n.edges.length)]?.id;c.stuck=0;c.speed=0;c.longAccel=0;c.driver=.8+Math.random()*.35;c.plannedEdge=null;resetGroundMotion(c);if(c.target!==undefined){const t=graph.nodes[c.target];c.yaw=Math.atan2(t.x-c.x,t.z-c.z);}const safe=dryRoad(c,c.spec,c);if(!safe){c.mesh.visible=false;return;}c.yaw=safe.yaw;c.road=safe.segment.road;const reverse=c.road.oneway===-1;c.prev=reverse?safe.segment.b:safe.segment.a;c.target=reverse?safe.segment.a:safe.segment.b;const count=laneCount(c.road);c.lane=(cars.indexOf(c)+Math.floor(state.elapsed/7))%count;c.desiredLane=c.lane;c.laneOffset=laneOffset(c.road,c.lane);const positions=[c.laneOffset,laneOffset(c.road,0),0].map(offset=>({x:safe.x+Math.cos(c.yaw)*offset,z:safe.z-Math.sin(c.yaw)*offset,offset})),spot=positions.find(p=>!vehicleBlocked(p.x,p.z,c.yaw,world.collision,c.spec,terrain.height(p.x,p.z))&&terrain.dry(p.x,p.z,c.spec.width/2)&&![...cars,...cops].some(other=>other!==c&&other.mesh.visible&&dist(other,p)<(c.spec.length+other.spec.length)/2+2));if(!spot){c.mesh.visible=false;return;}c.x=spot.x;c.z=spot.z;c.laneOffset=spot.offset;c.y=terrain.roads.sample(c.road,c.x,c.z)+.05;c.mesh.visible=true;poseVehicle(c);}
...
const TAXI_FACTS=[
 ...
 'Padova è attraversata da una rete di canali, ponti e antiche vie d’acqua.',
 'La Torre dell’Orologio mostra anche fasi lunari e segni zodiacali.',
 'Il Portello era una porta fluviale e oggi è uno dei cuori universitari.',
 'Le mura rinascimentali di Padova formano un circuito lungo circa undici chilometri.',
 'Il Caffè Pedrocchi è celebre per essere stato a lungo aperto giorno e notte.'
];
let taxiFactIndex=0;
function nextTaxiFact(){taxiFactIndex=(taxiFactIndex+1)%TAXI_FACTS.length;$('taxiFact').textContent=TAXI_FACTS[taxiFactIndex];}
function createTaxiDriver(){
 const driver=createPerson('#b9a46e',77),glass=new THREE.MeshStandardMaterial({color:'#101719',roughness:.25,metalness:.35});
 for(const side of [-1,1]){const lens=new THREE.Mesh(new THREE.BoxGeometry(.15,.085,.035),glass);lens.position.set(side*.09,1.64,.19);driver.add(lens);}
 const bridge=new THREE.Mesh(new THREE.BoxGeometry(.06,.025,.04),glass);bridge.position.set(0,1.64,.19);driver.add(bridge);driver.visible=false;scene.add(driver);return driver;
}
...
function movePlayer(dt){if(taxi?.phase==='loading'){state.speed=0;return;}if(incidents?.recovery){state.speed=0;if(state.elapsed>=incidents.recovery.until){incidents.recovery=null;if(state.respawnHome){respawnAtVilla();return;}if(state.car)state.car.mesh.visible=true;recover();toast('Vehicle recovered nearby. Continue exploring.');}return;}if(waterRecovery.active){falling(dt);return;}if(state.mode==='foot'&&state.health<=0){explodePlayer('Personaggio neutralizzato');return;}const f=(keys.has('KeyW')||keys.has('ArrowUp')?1:0)-(keys.has('KeyS')||keys.has('ArrowDown')?1:0),turn=(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)-(keys.has('KeyD')||keys.has('ArrowRight')?1:0),boost=keys.has('ShiftLeft')||keys.has('ShiftRight');
 ...
 car.x=state.x;car.z=state.z;car.yaw=state.yaw;car.health=state.health;car.speed=state.speed;car.y=state.y;poseVehicle(car);car.mesh.rotation.z=-(car.steerInput||0)*(car.jump?.airborne?.3:1)*Math.min(isBike(car.style)||spec.bike?.36:.025,Math.abs(state.speed)*(isBike(car.style)||spec.bike?.018:.001));state.distance+=Math.abs(state.speed)*dt;
 }else{
...
function updateTraffic(dt){for(const c of cars){
 ...
 const nx=c.x+Math.sin(newYaw)*c.speed*step,nz=c.z+Math.cos(newYaw)*c.speed*step;
...
function interact(){if(taxiCanTalk())openTaxiMenu();else toggleVehicle();}
