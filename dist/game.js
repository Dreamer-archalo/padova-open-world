...
import {watercraftStep} from './nautical-catalog.js';
...
function hospitalRoofRespawnPoint(){
 ...
 if(Math.abs(state.y-site.roofY)>5||!pointInside(state.x,state.z,site.polygon))return null;
 let lift=null,best=Infinity;
 for(const e of lifts.elevators||[]){const d=Math.hypot(state.x-e.roof.x,state.z-e.roof.z);if(d<best){best=d;lift=e;}}
 if(!lift)return null;
 return {x:lift.roof.x,z:lift.roof.z,y:lift.roofY+.24,yaw:lift.yaw,roofY:lift.roofY};
}
function respawnAtHospitalRoof(){
 ...
 waterRecovery.remember(state,terrain);world.update(state.x,state.z,true);camera.position.set(state.x-9,state.y+7,state.z-11);followYaw=state.yaw;cameraRig.reset(state.yaw);routeTo(currentTarget());
 if(gameplay)gameplay.graceUntil=Math.max(gameplay.graceUntil||0,state.elapsed+5);
 toast('Pista moto Monoblocco · respawn sul tetto.',4);return true;
}

function recover(fromWater=false){
 if(incidents?.recovery)return false;if(state.car?.spec?.watercraft&&gameplay?.nautical){const c=state.car,d=gameplay.nautical.activeDock;state.car=null;gameplay.remove(c);gameplay.nautical.playerBoat=null;return gameplay.nautical.launch(c.style,d,c.waterColor);}state.parachuting=false;if(parachute)parachute.visible=false;fromWater=fromWater===true;state.knockX=state.knockZ=state.spin=0;
...
function explodePlayer(reason){if(incidents?.recovery)return;state.respawnWaterDock=state.wanted!==5&&state.car?.spec?.watercraft&&gameplay?.nautical?{id:state.car.style,dock:gameplay.nautical.activeDock,color:state.car.waterColor}:null;state.respawnHospitalRoof=hospitalRoofRespawnPoint();state.respawnHome=!state.respawnHospitalRoof&&(state.wanted===5||state.mode==='foot');if(!incidents?.explode(state,state.elapsed,reason)){state.respawnHospitalRoof=null;return;}state.speed=0;state.health=0;state.spin=state.knockX=state.knockZ=0;keys.clear();if(state.car){state.car.health=0;state.car.mesh.visible=false;}if(state.respawnHome){cancelMission(false);clearPolice();}toast(reason+(state.respawnHospitalRoof?' · Respawn sul tetto…':state.respawnHome?' · Ritorno alla villa…':' · Recovering nearby…'),2);}
...
function movePlayer(dt){if(taxi?.phase==='transition'){state.speed=0;return;}if(incidents?.recovery){state.speed=0;if(state.elapsed>=incidents.recovery.until){incidents.recovery=null;if(state.respawnHospitalRoof){respawnAtHospitalRoof();return;}if(state.respawnWaterDock&&gameplay?.nautical){const p=state.respawnWaterDock,old=state.car;state.respawnWaterDock=null;state.car=null;if(old)gameplay.remove(old);gameplay.nautical.playerBoat=null;gameplay.nautical.launch(p.id,p.dock,p.color);return;}if(state.respawnHome){respawnAtVilla();return;}if(state.car)state.car.mesh.visible=true;recover();toast('Vehicle recovered nearby. Continue exploring.');}return;}if(waterRecovery.active){falling(dt);return;}if(state.mode==='foot'&&state.health<=0){explodePlayer('Personaggio neutralizzato');return;}const f=(keys.has('KeyW')||keys.has('ArrowUp')?1:0)-(keys.has('KeyS')||keys.has('ArrowDown')?1:0),turn=(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)-(keys.has('KeyD')||keys.has('ArrowRight')?1:0),boost=keys.has('ShiftLeft')||keys.has('ShiftRight');
 ...
 if(state.mode==='car'&&state.car?.spec?.watercraft){if(state.health<=0){explodePlayer('Imbarcazione distrutta');return;}const navigation=watercraftStep(state,keys,dt,terrain);state.distance+=Math.abs(state.speed)*dt;if(navigation?.blocked&&state.elapsed>(gameplay?.nautical?.lastWarning||0)+2){if(gameplay?.nautical)gameplay.nautical.lastWarning=state.elapsed;toast(navigation.lowBridge?'Ponte troppo basso per questa barca.':'Canale troppo stretto: scegli acque più larghe.',2);}return;}
