...
Trams.prototype.update=function(dt,time,player,actors,kick){
 ...
 for(const tram of this.vehicles){if(tram.sleeping)continue;
  ...
  for(let i=0;i<2;i++){const pose=sampleRail(tram.route,tram.d-i*8.65*tram.direction),g=tram.cars[i],y=surfaces.getTramHeight(pose.road,pose.x,pose.z,g.position.y),surfaceType=surfaces.getSurfaceType(pose.x,pose.z,y);g.visible=dist(pose,player)<700;g.position.set(pose.x,y,pose.z);g.rotation.y=pose.yaw+(tram.direction<0?Math.PI:0);g.userData.surfaceType=surfaceType;g.userData.isTram=true;g.userData.supportsTerrainSnap=!['bridge','tunnel','tram-structure'].includes(surfaceType);if(!g.visible)continue;
