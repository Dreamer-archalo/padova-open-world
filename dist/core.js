...
export function collides(x,z,r,index){for(const b of index.near(x,z,r+1)){if(x+r<b.minX||x-r>b.maxX||z+r<b.minZ||z-r>b.maxZ)continue;if(pointInside(x,z,b.p))return b;for(let i=0;i<b.p.length;i++){const n=nearestOnSegment(x,z,b.p[i],b.p[(i+1)%b.p.length]);if(Math.hypot(x-n.x,z-n.z)<r)return b;}}return null;}
...
export function makeRoadGraph(roads){const nodes=[],lookup=new Map(),segments=[],index=new SpatialIndex(90);function node(p){const k=p[0].toFixed(1)+','+p[1].toFixed(1);if(!lookup.has(k)){lookup.set(k,nodes.length);nodes.push({x:p[0],z:p[1],edges:[]});}return lookup.get(k);}
  for(const road of roads){if(['footway','path','cycleway','steps','track'].includes(road.k))continue;for(let i=1;i<road.p.length;i++){const a=node(road.p[i-1]),b=node(road.p[i]);const d=dist(nodes[a],nodes[b]);if(d<.1)continue;nodes[a].edges.push({id:b,d});nodes[b].edges.push({id:a,d});const s={a,b,road};segments.push(s);index.add(s,Math.min(nodes[a].x,nodes[b].x),Math.min(nodes[a].z,nodes[b].z),Math.max(nodes[a].x,nodes[b].x),Math.max(nodes[a].z,nodes[b].z));}}
  ...
  for(let i=0;i<nodes.length;i++){if(component[i]>=0)continue;const id=sizes.length,q=[i];component[i]=id;for(let j=0;j<q.length;j++)for(const e of nodes[q[j]].edges)if(component[e.id]<0){component[e.id]=id;q.push(e.id);}sizes.push(q.length);}
