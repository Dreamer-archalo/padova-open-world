...
if(canvas&&wrap){
 ...
 canvas.addEventListener('pointerdown',e=>{if(scale<=1)return;drag={id:e.pointerId,x:e.clientX,y:e.clientY,tx,ty};moved=false;canvas.setPointerCapture?.(e.pointerId);});
 canvas.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;const dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(Math.hypot(dx,dy)>4)moved=true;tx=drag.tx+dx;ty=drag.ty+dy;paint();});
 ...
 if(mapDialog)new MutationObserver(()=>{if(mapDialog.open){scale=1;tx=ty=0;paint();}}).observe(mapDialog,{attributes:true,attributeFilter:['open']});
 paint();
}
