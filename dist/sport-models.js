...
export function createWedgeCar(color,w=2,l=4.5,h=1.18,bigWing=false){
 ...
 box(g,'#1f353e',0,(h+.65)/2,-l*.1,w*.64,h-.65,l*.34);box(g,color,0,h-.04,-l*.12,w*.67,.065,l*.27);
 ...
 for(const side of [-1,1]){
  ...
  if(bigWing){const wingTop=h+.06,postBottom=.72;box(g,'#252f36',side*w*.3,(wingTop+postBottom)/2,-l*.36,.08,Math.max(.16,wingTop-postBottom),.15);}
 }
 box(g,'#253035',0,.25,l*.45,w*.98,.055,.16);
 if(bigWing){const wingY=h+.02;box(g,'#222e35',0,wingY,-l*.39,w*.94,.08,.5);for(const side of [-1,1])box(g,color,side*w*.455,wingY,-l*.39,.055,.18,.52);}
