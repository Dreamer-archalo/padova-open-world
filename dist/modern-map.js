...
// Only landmarks whose authored geometry relies on the complete OSM footprint are
// protected. A generic named building must not be allowed to occupy a road corridor.
...
function corridorMargin(road){
 // Match the widest geometry rendered around the centreline. Normal urban roads
 // can add ~1.2 m of sidewalk; bridge parapets sit outside the carriageway.
 if(/^(motorway|motorway_link|trunk|trunk_link|track)$/.test(road.k))return .55;
 return road.crossing?1.1:1.35;
}
...
export function modernFootprints(buildings,terrain,{force=false}={}){const result=[];let corrected=0;
 ...
 // active; initial-loader then prepares only the streamed chunks with force:true.
 ...
 for(const building of buildings){if(building.authoredLandmark||protectedLandmarks.has(building.n)){result.push(building);continue;}
  const b=bounds(building.p),width=b.maxX-b.minX,depth=b.maxZ-b.minZ,radius=Math.max(width,depth)/2+7;let parts=[building.p],base=null;
  for(const s of terrain.roads.index.near(b.cx,b.cz,radius)){const road=s.profile.road;if(nonDriving.test(road.k))continue;const margin=corridorMargin(road),pad=road.w/2+margin+.9;if(!overlapsSegmentBox(b,s,pad))continue;
   const dx=s.b[0]-s.a[0],dz=s.b[1]-s.a[1],d=Math.hypot(dx,dz);if(d<.01)continue;const q=nearestOnSegment(b.cx,b.cz,s.a,s.b);if(Math.hypot(b.cx-q.x,b.cz-q.z)>radius+road.w/2+margin)continue;
   ...
   const w=road.w/2+margin,nx=-dz/d*w,nz=dx/d*w,extend=Math.max(2.4,margin+1),ax=s.a[0]-dx/d*extend,az=s.a[1]-dz/d*extend,bx=s.b[0]+dx/d*extend,bz=s.b[1]+dz/d*extend;
