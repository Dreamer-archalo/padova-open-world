...
import {refineMandriaRoads} from './villa-mandria-v11-road-quality.js';
...
function finaliseMandria(map,relocation){if(!relocation)return;verifyParkAndVillaMap(map,relocation);privatizeMandriaRoads(map);refineMandriaRoads(map);}
export function prepareGameplayMap(map){
  ...
  if(map.gameplay.airportRoadIntegration){finaliseMandria(map,relocation);return map;}
  ...
  if(!entry){map.gameplay.airportRoadIntegration={status:'missing-entrance',connected:false};finaliseMandria(map,relocation);return map;}
  ...
  finaliseMandria(map,relocation);
  return map;
}