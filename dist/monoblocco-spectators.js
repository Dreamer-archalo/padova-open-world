...
import {roofClear,onTrack} from './monoblocco-track.js';
import {resolveHospital} from './hospital-rooftop-easter-egg.js';

// Spectators are authored objects, not generic city pedestrians: the street
// population recycler must never teleport them off the hospital roof.
...
function spectator(root,p,y,track,index){
 ...
 // Canvas is optional in headless tests; the physical spectator still exists.
...
function install(game){if(perGame.has(game))return;const site=resolveHospital(game);if(!site)return;const layout=game.__monobloccoSpectatorLayout||null;
 // Reuse the installed roof track when possible, rather than guessing a second
 // track geometry from the hospital bounding box.
 const roof=game.scene.children.find(o=>o.name==='ospedale-monoblocco-entire-roof-trial');if(!roof||!layout)return;
