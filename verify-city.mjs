...
// Uninterrupted 100+ metre run at 350–396 km/h on an audited real straight.
reset();t.state.speed=100;t.keys.add('Tab');const highStart={x:t.state.x,z:t.state.z};for(let i=0;i<60;i++){t.state.elapsed+=1/60;t.movePlayer(1/60);assert(!t.waterRecovery.active&&!city.incidents.recovery);assert.equal(t.state.health,100);}assert(dist(t.state,highStart)>100);const fastSpeed=t.state.speed;t.keys.delete('Tab');t.movePlayer(1/60);assert(t.state.speed>fastSpeed-1,'releasing turbo must decelerate gradually');
// Keep the vehicle on the verified straight section for the sustained-speed timing bench.
...
assert(t.world.details.tadi.userData.facades>5);t.world.update(-565,-55,true);for(let i=0;i<8000&&(t.world.queue.length||t.world.pendingBuild);i++)t.world.update(-565,-55);assert(!t.world.pendingBuild&&!t.world.queue.length,'visual streaming completes: '+JSON.stringify(t.world.streaming?.metrics));let plants=0;for(const group of t.world.loaded.values())for(const plant of group.userData.vegetation||[]){assert(plant.authored?insideArea(VILLA,plant.x,plant.z):city.districts.canPlant(plant.x,plant.z,t.terrain));plants++;}assert(plants>100);
