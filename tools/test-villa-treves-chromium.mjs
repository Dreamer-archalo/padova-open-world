...
try{
 ...
 await page.waitForFunction(()=>!document.getElementById('playingUI').hidden&&globalThis.__villaTest?.villaEstate?.root?.visible,null,{timeout:30000});
 ...
 const view=await page.evaluate(async()=>{
  ...
  const assets=g.villaEstate.root.children.length;
  return {ready:!!g.villaEstate,visible:g.villaEstate.root.visible,assets,
 ...
 const count=await page.evaluate(()=>globalThis.__villaTest.villaEstate.root.visible);
 assert(count,'estate disappeared while approaching garage');
