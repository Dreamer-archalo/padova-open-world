...
assert(HANGAR_SECTIONS.find(s=>s.id==='water')?.enabled===true,'new drivable fleet must enable boat category');
assert(ids.length>=52,'do not lose the existing vehicle registry');
assert(ids.every(id=>['air','land','urban','water'].includes(hangarSection(id,VEHICLES[id]))),'each model must have one playable top-level category');
...
const examples=['mito','cinquecento','saetta','fulmine','mil-tank-heavy','mil-truck-carrier','bicycle','kick-scooter','falco','rondone','airport-jet','airport-cargo','airport-michelangelo','boat-electric','boat-rib','boat-sport'];
...
assert.equal(aircraft.length,17,'verify sixteen existing aircraft plus Michelangelo');
const water=ids.filter(id=>hangarSection(id)==='water');assert(water.length>=7,'require seven or more Padova-compatible river craft');
...
console.log('PASS hangar sections: '+JSON.stringify({total:ids.length,air:aircraft.length,land:ids.filter(id=>hangarSection(id)==='land').length,urban:ids.filter(id=>hangarSection(id)==='urban').length,boats:water.length,modelFactories:examples.length,aircraftPlaceholderRegressions:aircraft.length}));
