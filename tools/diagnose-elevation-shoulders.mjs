...
const report={samples:0,violations:0,ordinary:0,independentLevel:0,bridgeFlag:0,layered:0,maxTransition:0,maxGap:0,top:[]};
for(const profile of terrain.roads.profiles.values()){
 ...
 for(let i=1;i<profile.points.length;i++){
  ...
  const mx=(a.x+b.x)/2,mz=(a.z+b.z)/2,yaw=Math.atan2(dx,dz),nx=Math.cos(yaw),nz=-Math.sin(yaw),deck=terrain.roads.sample(road,mx,mz);
  for(const side of [-1,1]){
   ...
   report.violations++;const independent=!!road.b||Math.abs(Number(road.layer)||0)>0;if(independent)report.independentLevel++;else report.ordinary++;if(road.b)report.bridgeFlag++;if(Math.abs(Number(road.layer)||0)>0)report.layered++;
   report.top.push({road:road.n||road.k,k:road.k,b:!!road.b,layer:Number(road.layer)||0,gap:+gap.toFixed(3),transition:+transition.toFixed(3),x:+mx.toFixed(1),z:+mz.toFixed(1)});
  }
 }
}
