...
import {VILLA,HOME,areaPoint,gameplaySpawns,gameplayStructures,gameplayVegetation} from '../dist/gameplay-areas.js';
...
assert(VILLA_GARAGE.w>=38&&VILLA_GARAGE.d>=38&&VILLA_GARAGE.roofHeight>9,'hangar fits aircraft and articulated trucks');
assert(VILLA_GARAGE.v-VILLA_GARAGE.d/2>=8,'hangar south wall clears original villa wing');
assert(VILLA_GARAGE.v+VILLA_GARAGE.d/2<49,'hangar north wall clears villa perimeter');
...
const display=areaPoint(VILLA,24,27);
...
const trees=gameplayVegetation(terrain),g=VILLA_GARAGE;
assert(trees.every(t=>{const u=t.x-VILLA.x,v=t.z-VILLA.z;return !(u>g.u-g.w/2-2&&u<g.u+g.w/2+2&&v>g.v-g.d/2-2&&v<g.v+g.d/2+2);}),
 'no decorative tree may grow through hangar roof or aircraft');
const spawns=gameplaySpawns().filter(p=>p.name===VILLA_PUBLIC_NAME);
assert.equal(spawns.length,7,'all seven existing vehicles retained in west parking');
for(const p of spawns){
 ...
 assert(p.x<VILLA.x,'old parked vehicles must not overlap east hangar');
}
...
assert(meshCount>30&&meshCount<130&&shadows===0,'bounded detail without shadow cost');
...
assert.equal(welcomes,1,'welcome not repeated each frame');
...
console.log('PASS Mandria hangar: aircraft fit, no wing/trees overlap, seven vehicles, driveway, real roof/walls and visual LOD');
