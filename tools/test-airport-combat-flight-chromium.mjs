...
try{
 ...
 const dive=await page.evaluate(()=>{const g=globalThis.__flightTest,s=g.state;return {y:s.y,speed:s.speed,yaw:s.yaw,pitch:s.flightPitch,modelPitch:s.car.mesh.rotation.x,boost:s.car.airBoost,cruise:s.car.airCruise,ui:document.getElementById('flightPanel')?.textContent};});
 console.log('BLACKBIRD_DIVE '+JSON.stringify({before,dive}));assert(dive.pitch<-.1&&dive.y<before.y&&dive.speed>before.speed&&dive.modelPitch>0&&dive.yaw!==before.y,'independent dive, model pitch or Shift steering/throttle failed');
 assert(dive.ui.includes('CTRL PICCHIATA')&&dive.ui.includes('SHIFT TURBO'),'flight cockpit instructions missing');
 ...
 phase='military interception';const ready=await page.evaluate(()=>{const g=globalThis.__flightTest,c=g.interactiveAirport.extras.find(c=>c.style==='airport-interceptor');if(!c)return {error:'interceptor absent'};
  ...
  Object.assign(g.state,{mode:'foot',car:null,parachuting:false,x,z,y:g.terrain.height(x,z),speed:0,vy:0,wanted:0,health:100});c.speed=0;c.parked=true;c.health=100;c.mesh.visible=true;g.airStrikes=3;return {style:c.style};});
 ...
 await page.evaluate(()=>{const g=globalThis.__flightTest,s=g.state;s.y=g.terrain.height(s.x,s.z)+85;s.speed=60;s.car.y=s.y;s.car.speed=60;s.car.parked=false;g.pose(s.car);});
 ...
 console.log('JET_INTERCEPTION '+JSON.stringify(threat));assert(threat.jets>=1&&threat.wanted>=3,'repeated jet attacks did not summon hostile interceptors');
 ...
 console.log('PASS WebGL Blackbird boarding/dive/boost/shockwave, F falling, hostile jets and color-coded flight map');
