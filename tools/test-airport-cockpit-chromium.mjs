...
try{
 ...
 phase='real civilian plane boarding';const staged=await page.evaluate(async()=>{
 ...
 await page.waitForFunction(()=>document.body.dataset.flightActive==='true'&&document.body.dataset.flightMilitary==='false',null,{timeout:12000});
 const cockpit=await page.evaluate(()=>{const g=globalThis.__cockpitTest;return {militaryHud:!document.getElementById('flightCockpit').hidden,speed:document.getElementById('speed')?.textContent,minimap:getComputedStyle(document.querySelector('.hud.minimap')).display,driving:getComputedStyle(document.querySelector('.hud.driving')).display,brake:g.state.car.spec.brake,gear:document.getElementById('gear')?.textContent};});
 console.log('CIVILIAN_FLIGHT_BOARD '+JSON.stringify(cockpit));assert(!cockpit.militaryHud&&cockpit.minimap!=='none'&&cockpit.driving!=='none'&&cockpit.speed!==undefined&&cockpit.gear!==undefined&&cockpit.brake>=8,'civilian aircraft must keep regular speed, gears and minimap, not military holograms');
 await page.screenshot({path:'test-artifacts/airport-civilian-panel-after-boarding.png',timeout:20000});
 ...
 phase='civilian missile safety';const safety=await page.evaluate(async()=>{
  ...
  return {wanted:s.wanted,health:s.health,missiles:g.incomingMissiles.length,hostiles:g.airDefenders.length,bonus:g.extraDogfighters.length,warning:document.getElementById('flightWarning')?.hidden};
 });console.log('CIVILIAN_SAFETY '+JSON.stringify(safety));assert(safety.wanted===0&&safety.health===100&&safety.missiles===0&&safety.hostiles===0&&safety.bonus===0&&safety.warning,'zero-star civilian got an incoming missile');
 ...
 assert.equal(errors.length,0,'browser JS errors '+errors.join(' | '));console.log('PASS WebGL civilian speed, gears, minimap, S reverse, zero-star safety and turbo speed sanity');
}catch(error){console.error('CIVILIAN_FLIGHT_WEBGL_FAIL '+phase+' '+(error.stack||error));console.error('JS_ERRORS '+JSON.stringify(errors.slice(-15)));try{await page.screenshot({path:'test-artifacts/airport-cockpit-failure.png',timeout:15000});}catch{}process.exitCode=1;}finally{await browser.close();}
