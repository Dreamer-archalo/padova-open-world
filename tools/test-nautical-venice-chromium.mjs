...
try{
 ...
 await page.keyboard.down('w');await page.evaluate(()=>{for(let i=0;i<42;i++)globalThis.__veniceWorld.boats.step(.05);});await page.keyboard.up('w');await page.waitForTimeout(250);
 const end=await page.evaluate(()=>{const s=globalThis.__veniceWorld.boats.current;return {x:s.x,z:s.z,speed:s.speed};});
 console.log('VENICE_BROWSER_DIAGNOSTICS '+JSON.stringify({stats,start,end,position:await page.evaluate(async()=>{const g=globalThis.__veniceWorld,b=g.boats.current,{canalAt}=await import('./venice-boats.js');return {dock:(()=>{const d=g.boats.docks.find(x=>x.id===b.dock);return {x:d.x,z:d.z,width:d.width,yaw:d.yaw,name:d.name};})(),navStart:canalAt(g.data,b.x,b.z,1.65),yaw:b.yaw,near:g.data.water.length};})}));
