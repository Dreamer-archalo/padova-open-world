...
async function enterSection(id){
 await page.locator(`#hangarSections [data-section="${id}"]`).click();
...
try{
 ...
 sections:[...document.querySelectorAll('#hangarSections [data-section]')].map(b=>b.dataset.section)}));
 console.log('HANGAR_MENU '+JSON.stringify(menu));
 assert(menu.cards>=35&&menu.hasBicycle&&menu.hasScooter&&menu.hasAircraft&&menu.thumbnails&&['land','air','urban'].every(s=>menu.sections.includes(s)),'catalogue image/name or category incomplete');
 await page.screenshot({path:'test-artifacts/mandria-hangar-menu.png',timeout:25000});
 phase='stage first car';await enterSection('land');await page.locator('#hangarSearch').fill('mito');
