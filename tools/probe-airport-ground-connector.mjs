...
const candidates=g.segments.filter(s=>s.connected&&!s.road.b&&!s.road.tunnel&&!s.road.crossing&&!Number(s.road.layer)&&!['private','no'].includes(s.road.access)&&['secondary','tertiary','residential','unclassified','service','primary'].includes(s.road.k)).map(s=>{
...
console.log('CANDIDATE',JSON.stringify({road:road.n,roadKind:road.k,access:road.access,bridge:road.b,layer:road.layer,point:p,distanceToApproach:chosen.d,entryPoint:entry,original:old},null,2));
// Temporarily split the *actual* road geometry at the projected junction. No
// changes are written to source, allowing a rigorous proof before implementation.
...
assert(index>0,'Could not identify exact source segment for a reversible topology split');
...
const parts=joined.segments.filter(s=>s.road===entrance);assert(parts.length===2&&parts.every(s=>s.connected),'Probe road remains disconnected');
const route=roadRoute({x:a.x,z:a.z},AIRPORT_GATE,joined,{maxMs:2000,maxSteps:100000});assert(route.length>=3,'Probe cannot route from city to airport');
...
console.log('PASS provisional connection topology, taxi, collision, water and grade. Requires separate visual map confirmation before release.');
